...
Die Methode generateTestCase() ruft eine Factory auf, welche aus einer XML-Datei einen Testcase generiert. Diese Factory wird unter dem Kapitel Testcase-Factory genauer beschrieben.
\paragraph
Alle Konfigurationsparameter welche der Server braucht, sind in einer Konfigurationsdatei abgelegt. Beim Start des Servers wird also diese Konfigurationsdatei in der Methode "readConfig()" ausgelesen. Darin sind unter anderem die Netzwerkports und die Ip-Adressen der Clients definiert. 
Aus den für den Client wichtigen Informationen wird ein Configuration-Objekt erzeugt. Dieses Objekt wird dann bei initialize an die Clients übergeben. Diese brauchen die im Objekt gespeicherten Informationen, um eine funktionierende RMI-Verbindung mit dem Server herstellen zu können.
