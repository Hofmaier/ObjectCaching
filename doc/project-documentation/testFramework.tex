...
\item Beim Aufruf der "setReady()" Methode auf dem Server wird nun geprüft, ob alle Clients bereit sind. Haben sich alle Clients als bereit für den Testlauf gemeldet, startet der Server auf jedem Client den Testlauf über die Methode "startTest()".
\item Zu diesem Zeitpunkt wartet der setReady Thread des letzten Clients immer noch auf die Rückkehr des Aufrufs.
\item Gleichzeit wird auf dem letzten Client ebenfalls die startTest() Methode aufgerufen. Dadurch bestehen beim letzten Client der sich beim Server gemeldet hat kurzzeit zwei Threads.
\item Ist die Methode setReady() erfolgreich durchlaufen, kehrt der Aufruf zum Client zurück daraufhin beendet der Thread auf dem Client, welcher die setReady() Methode gestartet hat.
\item Gleichzeitig hat der startTest() Thread bereits begonnen mit dem abarbeiten des Szenarios. Da nun der Thread vom setReady() beendet wird, wird dem Szenario Thread die CPU-Ressource weggenommen. Dies führt nun zu einer Verlängerung der Ausführungsdauer der Aktion.
\end{enumerate}
