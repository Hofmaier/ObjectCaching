...
Die Arbeit wurde durch Mercury motiviert, unserer Ziele sind aber genereller definiert.
...
\begin{itemize}
\item Kann ein Objekt von mehreren Clients modifiziert werden, so muss bei einer Schreiboperation garantiert werden, dass ein Client vor der Schreiboperation den aktuellen Wert aus dem Objekt gelesen hat. Wird dies nicht geprüft kann es dazu führen das Änderungen, welche ein anderer Client zwischenzeitlich gemacht hat, verloren gehen.
\end{itemize}
...
\section{Sitzungen}
Mit dem Betreuer dieser Arbeit findet eine wöchentliche Sitzung statt. Dabei werden die Fortschritte besprochen und falls vorhanden, offene Fragen diskutiert. Die Länge der Sitzung ist daher variabel, je nachdem ob und wieviele offene Fragen und Probleme bestehen. Die diskutieren Punkte sowie die Entscheidungen werden in einem Sitzungsprotokoll schriftlich festgehalten.
